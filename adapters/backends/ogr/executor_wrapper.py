...
class OGRFilterExecutor(FilterExecutorPort):
    ...
    def build_expression(
        ...
        **kwargs
        ...
        """
        Build filter expression compatible with GeometricFilterBackend interface.
        
        v4.2.0: Added for compatibility with legacy adapter after before_migration removal.
        Returns empty string as OGR doesn't use SQL expressions (uses QGIS Processing).
        """
        # OGR doesn't build SQL expressions - it uses QGIS Processing
        # Return empty string to signal "no SQL needed"
        logger.debug(f"[OGR] build_expression() called (returns empty - OGR uses Processing)")
        return ""
    ...
    def apply_filter(
        ...
        """
        Apply filter to layer compatible with GeometricFilterBackend interface.
        
        v4.2.0: Added for compatibility with legacy adapter after before_migration removal.
        For OGR, this is typically handled via QGIS Processing, not subset strings.
        """
        if not expression:
            # OGR uses processing, not subset strings
            logger.debug(f"[OGR] apply_filter() with empty expression (normal for OGR)")
            return True
        
        # Fallback: try to apply as subset string if expression provided
        return self.apply_subset_string(layer, expression)
    ...
    def supports_layer(self, layer) -> bool:
        ...
        # OGR is universal fallback - supports everything except memory/postgresql/spatialite
        return provider not in ('memory', 'postgres', 'spatialite')
