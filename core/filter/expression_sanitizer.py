...
def sanitize_subset_string(subset_string: str) -> str:
    ...
    for pattern in coalesce_patterns:
        ...
        if match:
            logger.info(
                f"FilterMate: Removing invalid coalesce expression: "
                f"'{match.group()[:60]}...'"
            )
    ...
    if match:
        logger.info(
            f"FilterMate: Removing SELECT CASE style expression: '{match.group()[:80]}...'"
        )
        sanitized = re.sub(
    ...
    for pattern in case_patterns:
        ...
        if match:
            ...
            if re.search(r'\bTHEN\s+(true|false)\b', matched_text, re.IGNORECASE):
                logger.info(
                    f"FilterMate: Removing invalid CASE/style expression: "
                    f"'{matched_text[:60]}...'"
                )
