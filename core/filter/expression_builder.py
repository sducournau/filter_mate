...
class ExpressionBuilder:
    ...
    def build_backend_expression(
        ...
        try:
            ...
            logger.debug(f"build_backend_expression: backend={backend_name}, "
                        f"predicates={self.current_predicates}, "
                        f"source_geom_type={type(source_geom).__name__}")
            ...
            if source_filter:
                logger.debug(f"Source filter prepared: {len(source_filter)} chars")
            elif backend_name == 'PostgreSQL':
                logger.warning("source_filter is empty - PostgreSQL EXISTS will query entire source table")
    ...
    def _prepare_source_filter(self, backend_name: str) -> Optional[str]:
        ...
        logger.debug("=" * 80)
        logger.debug(f"      📋 ATTEMPT 1: task_parameters['task']['features']")
        ...
        if task_features and len(task_features) > 0:
            ...
            if not are_qgs_features:
                ...
                logger.debug(f"         ⚠️ task_features contains values, not QgsFeature objects!")
                logger.debug(f"         🔍 Checking custom_expr from task_parameters...")
                    ...
                    if where_match:
                        ...
                        if '__source' in where_clause.lower():
                            ...
                            if parsed_where['can_optimize']:
                                ...
                                if source_filter:
                                    logger.debug(f"   ✅ Advanced optimization succeeded!")
