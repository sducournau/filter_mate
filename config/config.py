...
def reset_config_to_defaults():
    ...
    Copies config.default.json to config.json and reloads.
    ...
    try:
        plugin_dir = ENV_VARS.get("PLUGIN_DIR", "")
        config_json_path = ENV_VARS.get("CONFIG_JSON_PATH")
        default_config_path = os.path.join(plugin_dir, "config", "config.default.json")
        ...
        ENV_VARS["CONFIG_DATA"] = default_config
...
def save_config_value(key: str, value, section: str = "APP"):
    """
    Save a single configuration value to config.json.
    
    Args:
        key: Configuration key (e.g., 'auto_activate', 'theme')
        value: Value to save
        section: Config section (default: 'APP')
    ...
    try:
        ...
        if not config_json_path:
            QgsMessageLog.logMessage(
                "Config path not initialized. Call init_env_vars() first.",
                "FilterMate",
                Qgis.Warning
            )
            return False
        
        # v5.0: Mapping from ConfigurationPage keys to config.json paths
        key_mapping = {
            'ui_profile': ('APP', 'DOCKWIDGET', 'UI_PROFILE'),
            'theme': ('APP', 'DOCKWIDGET', 'THEME'),
            'auto_activate': ('APP', 'AUTO_ACTIVATE'),
            'remember_filters': ('APP', 'OPTIONS', 'REMEMBER_FILTERS'),
            'auto_switch_exploring': ('APP', 'OPTIONS', 'AUTO_SWITCH_EXPLORING'),
            'show_advanced': ('APP', 'OPTIONS', 'SHOW_ADVANCED'),
            'experimental': ('APP', 'OPTIONS', 'EXPERIMENTAL'),
            'default_backend': ('APP', 'OPTIONS', 'DEFAULT_BACKEND'),
            'raster_sampling': ('APP', 'RASTER', 'DEFAULT_SAMPLING'),
            'raster_clip_op': ('APP', 'RASTER', 'DEFAULT_CLIP_OP'),
            'use_pyramids': ('APP', 'RASTER', 'USE_PYRAMIDS'),
            'cache_histogram': ('APP', 'RASTER', 'CACHE_HISTOGRAM'),
        }
        ...
        if key in key_mapping:
            ...
            # Set value (support both direct values and dict with 'value' key)
            if isinstance(current.get(path[-1]), dict) and 'value' in current.get(path[-1], {}):
                current[path[-1]]['value'] = value
            else:
                current[path[-1]] = value
        else:
            # Direct key in section
        ...
        QgsMessageLog.logMessage(
            f"Failed to save config value {key}: {e}",
            "FilterMate",
            Qgis.Warning
        )
        return False
