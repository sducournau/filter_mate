...
This file configures QGIS mocks that work for all style modules.
"""
...
# These mocks must be configured BEFORE any ui.styles imports


def _configure_qgis_mocks():
    """Configure QGIS mocks to work with all style modules."""
    ...
    # Configure QtCore with a proper QObject class that doesn't interfere
    # with multiple inheritance from StylerBase
    _qt_core_mock = MagicMock()
    
    # Create a minimal QObject class that works with multiple inheritance
    class MockQObject:
        """Mock QObject that works with StylerBase inheritance."""
        def __init__(self, *args, **kwargs):
            pass
    
    _qt_core_mock.QObject = MockQObject  # Use our mock class
